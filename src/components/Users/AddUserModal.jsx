import { X, User, Mail, Hash, Phone, UserPlus } from 'lucide-react'
import { useState } from 'react'
import { addUser } from '../../actions/Users/usersAction'
...
const AddUserModal = ({ onClose, onSuccess }) => {
  const [form, setForm] = useState({ name: '', email: '', empId: '', mobile: '' })
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    const allTouched = { name: true, email: true, empId: true, mobile: true }
    setTouched(allTouched)
    const errs = validate(form)
    setErrors(errs)
    if (Object.keys(errs).length > 0) return
    setApiError('')
    addUser(form, setLoading, () => {
      onSuccess({
        name:         form.name,
      ...
      <div className={styles.modal}>
        <div className={styles.modalHeader}>
          ...
          <div>
            ...
            <p className={styles.modalSub}>Create a new field executive account</p>
          </div>
        ...
        <form className={styles.form} onSubmit={handleSubmit} noValidate>
          ...
          <div className={styles.field}>
            ...
            <div className={`${styles.inputWrap} ${errors.empId && touched.empId ? styles.inputError : ''}`}>
              <Hash size={15} className={styles.icon} />
              <input name="empId" placeholder="Enter employee ID" value={form.empId} onChange={handleChange} onBlur={handleBlur} />
            </div>
            {errors.empId && touched.empId && <span className={styles.error}>{errors.empId}</span>}
          </div>
          ...
          <div className={styles.field}>
            ...
            <div className={`${styles.inputWrap} ${errors.mobile && touched.mobile ? styles.inputError : ''}`}>
              ...
              <input name="mobile" placeholder="Enter 10-digit mobile number" value={form.mobile} onChange={(e) => { if (/^\d*$/.test(e.target.value)) handleChange(e) }} onBlur={handleBlur} maxLength={10} inputMode="numeric" />
            </div>
